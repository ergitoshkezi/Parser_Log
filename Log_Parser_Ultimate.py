...
####________________________________________________________########################################
    ...
    def show_excel_data(self):
            ...
            for col_idx, column in enumerate(self.df.columns):
                ...
                data_frame.grid_columnconfigure(col_idx, weight=1, minsize=max_widths[col_idx]*8)  # Multiply by 8 for pixel width
...
###############################################################################################################
    ...
    def parse_log(self, log_file, file):
        ...
        refference=["0000-00-00 00:00:00.000000"]*5
        ref_line=[""]*5
        self.delta=[]
            ...
            for index, line in enumerate(self.log_lines):
                ...
                if timestamp_match:
                    ...
                    if self.previous_timestamp:
                        ...
                        if delta > refference[0]:
                            refference[0]=delta
                            ref_line[0]=line
                        elif delta > refference[1]:
                            refference[1]=delta
                            ref_line[1]=line
                        elif delta > refference[2]:
                            refference[2]=delta
                            ref_line[2]=line
                        elif delta > refference[3]:
                            refference[3]=delta
                            ref_line[3]=line
                        elif delta > refference[4]:
                            refference[4]=delta
                            ref_line[4]=line
    ...
    # Create popup window
        ...
        for j in range(0,5):
            ...
            text_widget.insert(tk.END, f"{ref_line[j]} \n time: {refference[j]}\n\n" )
